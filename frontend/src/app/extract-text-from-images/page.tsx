...
const OCR: React.FC = () => {
  ...
  const scribeIt = async (event: React.FormEvent) => {
    ...
    if(usingUrl){
      try {
        const response = await fetch('/apis/post-ocr-url', {
          method: "POST",
          headers: {
        ...
        setDescription(data.description["<OCR>"])
        setLoader(false)
        console.log("OCR result:", data.description["<OCR>"])
      ...
      try {
        const response = await fetch('/apis/post-ocr-image', {
          method: "POST",
          body: formData,
        });
        ...
        const ocrText = data.description["<OCR>"];
...
const hitTranslateApi = async (event: any, target: string) => {
  ...
  return (
    <>
      ...
      >
      <h1 className=" text-center mt-7 text-primary_color text-2xl font-semibold">Extract Text From Image</h1>
        <div
